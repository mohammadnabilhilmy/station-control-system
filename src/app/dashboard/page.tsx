      ...
      <main className="flex-1 min-w-0 flex flex-col h-screen bg-[#f8fafc]">
        ...
        <header className="bg-white border-b border-slate-200 px-4 md:px-8 py-4 sticky top-0 z-40 flex items-center justify-between">
          ...
          <div className="flex gap-2 items-center">
            <button onClick={() => setActiveTab(activeTab === 'dashboard' ? 'monitoring' : 'dashboard')} className="hidden sm:flex items-center gap-2 bg-blue-50 text-blue-600 px-4 py-2 rounded-xl font-black text-[10px] uppercase border border-blue-100 hover:bg-blue-100 transition-colors">
              <MonitorDot size={14} /> {activeTab === 'dashboard' ? 'Live Monitor' : 'Dashboard'}
            </button>

            <div className="flex gap-1 md:gap-2">
              <button onClick={exportToPDF} className="flex items-center gap-1.5 bg-red-50 text-red-600 px-3 md:px-4 py-2 rounded-xl font-black text-[9px] md:text-[10px] uppercase border border-red-100">
                <FileText size={14} /> <span className="hidden xs:block">PDF</span>
              </button>
              <button onClick={exportToExcel} className="flex items-center gap-1.5 bg-emerald-50 text-emerald-600 px-3 md:px-4 py-2 rounded-xl font-black text-[9px] md:text-[10px] uppercase border border-emerald-100">
            ...
            {activeTab === 'dashboard' ? (
              <>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                      ...
                      <div className="flex bg-slate-100 p-1 rounded-xl overflow-x-auto w-full sm:w-auto">
                        {(['all', 'high', 'pending'] as const).map((f) => (
                          <button key={f} onClick={() => setFilter(f)} className={`flex-1 sm:flex-none px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${filter === f ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400'}`}>
                            {f}
                          </button>
                        ))}
                      </div>
                  ...
                  <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl flex flex-col justify-center">
                    <div className="relative z-10">
                      <p className="text-blue-400 font-black text-[10px] tracking-[0.2em] uppercase mb-2">Operational Score</p>
                      <h2 className="text-2xl md:text-3xl font-black leading-tight">Efisiensi<br />Penyelesaian</h2>
                    </div>
                    <div className="relative z-10 py-6 md:py-10">
                      <div className="text-5xl md:text-7xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-blue-500">{stats.total > 0 ? Math.round((stats.resolved / stats.total) * 100) : 0}%</div>
...
function StatCard({ title, value, icon, color, isAlert = false }: any) {
  ...
  return (
    <div className={`p-5 md:p-6 rounded-[2.5rem] bg-white border border-slate-200 shadow-sm transition-all hover:shadow-xl ${isAlert ? 'ring-2 ring-red-500 animate-pulse' : ''}`}>
      <div className={`w-10 h-10 md:w-12 md:h-12 rounded-2xl flex items-center justify-center mb-4 md:mb-6 shadow-lg ${colorMap[color]}`}>{icon}</div>
      <div className="text-slate-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em]">{title}</div>
      <div className="text-3xl md:text-4xl font-black text-slate-800 mt-1 tabular-nums">{value}</div>
    </div>
  );
}
